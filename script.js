...
function findBox(filterBtn) {
  boxes.forEach((box) => {
    ...
    box.addEventListener("mouseenter", () => {
      // githubCodes.forEach((gitCode)=>{
      //    gitCode.classList.remove("hide")
      // })
    ...
    if (
      ...
      pagesTitles.forEach((projectTitle, i) => {
        ...
        if (pagesTitles[i].className.includes(sec.getAttribute("id"))) {
          ...
          //  pagesTitles[i].style.textShadow = "0";
        }
      });
    }
  });
    ...
    if (top >= offset && top < offset + Height) {
      ...
      lis.forEach((li) => {
        ...
        if (
          ...
          //  console.log(li)
          li.parentElement.style.backgroundColor = " #2b2c68";
        } else {
          li.parentElement.style.backgroundColor = "#26b7cd ";
        }
      });
    }
...
// let rebot = document.querySelector("spline-viewer #container #logo");
// console.log(rebot)
// rebot.style.display = "none";;

let githubCodes = document.querySelectorAll(".git");
console.log(githubCodes);
